...
module Herdis
  ...
  class Client
    ...
    class CannotDistribute < RuntimeError
      ...
      def message
        "#{@command.to_s.upcase} cannot be used in Herdis::Client because the keys involved need to be on the same server or because we cannot guarantee that the operation will be atomic."
      end
    end
    ...
    def node_for(key)
      @nodes[Digest::SHA1.hexdigest(key.to_s).to_i(16) % Herdis::Common::SHARDS]
    end
